defmodule ForthIbE.Interpreter do
  ...
  def interpret(tokens, dictionary, table) do
    #IO.inspect(tokens)	
	derivation = case _interpret(tokens, dictionary, table) do                
  ...
  defp _interpret([], dictionary, _table) do 	# завершение рекурсивной обработки  
  ...
  defp _interpret([:comment | tail], dict, table) do   # выполняется непосредственно в трансляции
	#IO.inspect(tail)
	{_comment_code, [:end_comment | new_tokens]} = Enum.split_while(tail, fn s -> s != :end_comment end)
	_interpret(new_tokens, dict, table)                # возвращаем 3 параметра ?!
  end

  #-------------------------
  # ( ! )
  #-------------------------
  defp _interpret([:create | tail], dictionary, table) do # выполняется непосредственно при трансляции
	{word_tokens, [:end_create | behind_tokens]} = Enum.split_while(tail, fn s -> s != :end_create end)
	[ word_name | content ] = word_tokens
	# IO.inspect(content)
	case interpret(content, dictionary, table) do     # конвертирование binary в атомы прямо в определении
  ...
  # ( if ... then ...else )
  #-------------------------
  defp _interpret(["if" | tail], dict, table) do							
...
#{if_virt, ["then" | behind_virt]} = Enum.split_while(virt, fn s -> s != "then"  end) # выделяем код до then
  ...
  defp _interpret(["else" | tail], dict, table) do								
	{virt,  dict, map} = _interpret(tail, dict, table)    # echo не учитываем
    #IO.inspect(virt)
  ...
  defp _interpret(["then" | tail],  dict, table) do								
	{virt_code,  dict, _map} = _interpret(tail,  dict, table)    # задний код за then
  ...
  # ( variable, abort", ." )
  #-------------------------
  defp _interpret(["variable", var_name | tail],  dictionary, table) do
	dict = add_var(dictionary, var_name, :unknown)
	_interpret(tail,  dict, table) #{virt_code,  dict, map} = 
  end

  defp _interpret(["abort\"" | tail],  dict, table) do
  ...
  defp _interpret([".\"" | tail],  dict, table)  do
    #IO.inspect(tail)
 	{quoted_tokens, [:end_quote | tokens]} = Enum.split_while(tail, fn s -> s != :end_quote  end)
	string = Enum.join(quoted_tokens, " ")
	#IO.puts(string)
  ...
  defp _interpret([token | tail],  dict, table)  when is_number(token) do
	{virt, dict, map} = _interpret(tail,  dict, table)
	{[token | virt], dict, map}
  end

  defp _interpret([token | tail],  dict, table)  when is_atom(token) do
	{virt, dict, map} = _interpret(tail,  dict, table)
	{[token | virt], dict, map}
  end

  defp _interpret([token | tail], dictionary, table) when is_binary(token) do # преобразуем токены и собираем virt_code, 
	found = get_atom(table, token)
    #IO.puts(token)
    #IO.inspect(table)
    #IO.inspect(found)
	case found do 
