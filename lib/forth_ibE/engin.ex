defmodule ForthIbE.Engin do
  ...
  def run(virt_code, data_stack, return_stack, dictionary) do
	# IO.puts("run")
	#IO.inspect(virt_code)
	result = next(virt_code, data_stack, return_stack, dictionary)
	#IO.inspect(virt_code)
	case result do
	  {:error, reason } -> {:error, reason }
	  {_virt_code, data_stack, return_stack, dictionary} -> #IO.inspect(virt_code)
  ...
  defp next([%{if: true_code, else: else_code} | tail], [cond | data_stack], return_stack, dictionary) do
	#IO.puts(cond)
	branch = case cond do
	  #false	-> else_code 
	  0		-> 	else_code
	...
	next( result_code,  data_stack, return_stack, dictionary)
  end
  ...
  defp next([first | tail], data_stack, return_stack, dictionary) when is_atom(first) do
	case apply(ForthIbE.Words, first, [tail, data_stack, return_stack, dictionary]) do
	  {:error, reason}	->	{:error, reason}
  ...
  defp next([first | tail], data_stack, return_stack, dictionary) when is_binary(first) do
	...
	case get(dictionary, first) do
	  {:words, word_code}	-> #IO.puts("word #{first}")
			next(word_code ++ tail,  data_stack, return_stack, dictionary)
	  _	->	next(tail, [first | data_stack], return_stack, dictionary)	
	end	
  end
end
